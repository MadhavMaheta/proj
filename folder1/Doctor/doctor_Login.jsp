...
<%
    String doc_username = request.getParameter("username");
    String doc_password = request.getParameter("password");
try{
    if(doc_username!=null && doc_password!=null){
    session.setAttribute("userName",doc_username);  
    Class.forName("com.mysql.jdbc.Driver");
    Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital_database","root",""); 
    Statement stmt=con.createStatement();
    ResultSet rs=stmt.executeQuery("SELECT * from doc_info WHERE username='"+doc_username+"' AND psswd='"+doc_password+"' ");
    if(rs.next()){
	response.sendRedirect("doc_apnHistory.jsp");
        return;
